...
class screenCapture:
    def __init__(self):
        if mode == "JIE_TU":
            ...
            self.bound = (35, 63, 35 + 420, 30 + 785)
    ...
    def _splitCapture(self, img):
        if type == "FRIEND_PK":
            ...
            questionImg = img.crop(
                (self.rpx(85), self.rpx(460 + 120), self.rpx(670), self.rpx(590 + 120))
            )
            optionsImg = img.crop(
                (self.rpx(85), self.rpx(590 + 120), self.rpx(670), self.rpx(1055 + 120))
            )
        if type == "SELF_PK":
            # 笔记本每日一答
            questionImg = img.crop(
                (self.rpx(85), self.rpx(460 - 20), self.rpx(670), self.rpx(590 - 20))
            )
            optionsImg = img.crop(
                (self.rpx(85), self.rpx(590 - 20), self.rpx(670), self.rpx(1055 - 20))
            )
